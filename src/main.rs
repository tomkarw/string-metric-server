...
async fn user_connected(ws: WebSocket, connections: Connections) {
...
impl DistanceRequest {
    fn new(string1: String, string2: String) -> Self {
        DistanceRequest { string1, string2 }
    }
}
...
async fn user_message(msg: Message, connections: &Connections) {
    if let Ok(msg) = msg.to_str() {
        ...
        let response = serde_json::to_string(msg).expect("failed to serialize response");
